...
- [x] **Testing stage in pipeline** — `verify` job (typecheck + lint + tests) gates `build-and-deploy` in `.github/workflows/deploy.yml`, and runs on PRs too. Lint gates on errors only; see `docs/state.md` for the remaining limits.
- [ ] **E2E tests**